...
function tipoJuego(id: string, v: string){
    document.getElementById(id).setAttribute("onclick",v)
}
function cambiarJuego(j: string){
    tipoJuego("0",j)
    tipoJuego("1",j)
    tipoJuego("2",j)
    tipoJuego("3",j)
...
function juegaPC(v: string){
    let num = 1
    let celda= Math.floor((Math.random() * 8) + 1)
    let id = celda.toString()
    if(verificarTablero()){
        while(get(id) != " " ){
...
function deshabilitarT(v: boolean){
